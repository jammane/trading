...
def test_upkeep_mt1_head_tail_block_cycle(tmp_path, monkeypatch):
    ...
    for _ in range(n_days):
        in74 = torch.randn(1, 74)   # Part C: [market37 ‖ portfolio37]
        actual_d = float(torch.randn(1).item()) * 500.0
        ret = upkeep_mt1_industry(ind, md, in74, actual_d, rolling_state=rolling)
