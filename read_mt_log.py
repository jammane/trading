...
def parse_log(path):
    ...
    fits_v8 = data_size > 0 and data_size % RECORD_SIZE_V8 == 0
    ...
    if not (fits_v1 or fits_v2 or fits_v3 or fits_v4 or fits_v5 or fits_v6 or fits_v7 or fits_v8):
        sys.exit(f'ERROR: data size {data_size} not divisible by any known record size '
                 ...
                 f'{RECORD_SIZE_V4}, {RECORD_SIZE_V5}, {RECORD_SIZE_V6}, {RECORD_SIZE_V7}, '
                 f'or {RECORD_SIZE_V8})')
    # Prefer newest format that fits
    if fits_v8:
        rec_size, fmt, ver = RECORD_SIZE_V8, RECORD_FMT_V8, 8
    elif fits_v7:
